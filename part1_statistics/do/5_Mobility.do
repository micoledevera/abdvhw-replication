...
global numestates = "1 3 5 10"		// How many years ahead mobility 3 5 10 15

global mathetgroup = `" "" "male agegp" male "' // Mobility conditional on
*global mathetgroup = `""'  // Mobility conditional on
*global mathetgroup = `"agegp"'  // Mobility conditional on
		...
		forvalues yr = `firstyr'/`yrmax'{		
			...
			*Load the data							
			...
			foreach t in "" "p"{ //Calculate the ranking in `yr' and `yrp'
				...
				replace `varx'rankt`t' = 100 if `varx'rankt`t' > 0.995 			
				
				replace `varx'rankt`t' = 99.5 if `varx'rankt`t' > 0.99 & `varx'rankt`t' <= 0.995 
