...
capture program drop  dnplot tsplt tspltEX tsplt2sc tspltAREALim tspltAREALimZero tspltAREALim2 gkswplot gkswplotax gkswplot_co_adj gkswplot_co_adj2 tspltAREALimPA5F tspltAREALimPA_nolegend
...
program tspltAREALimPA_nolegend
...
*Plot
	if "`28'" == "yes"{
		*With dashed line*
		...
		legend(off) graphregion(color(white)) /// Legend options 
...
*Plot
	if "`28'" == "yes"{
		...
		*Without dashed line*
		tw   (bar rece year, ylabel(,nogrid axis(1)) c(l) color(gray*0.5) yscale(off)) ///
...
program gkswplot_co_adj
	...
	*Plots
	...
	foreach yy in `ygroups'{
		...
		local cplots = "`cplots' (connected `yvar' `xvar' if cohort25 == `yy', yaxis(1)  yscale(alt) yscale(alt axis(2)) lpattern(`wpatt') lcolor(`wcolor') mfcolor(`wcolor'*0.25) mlcolor(`wcolor') msymbol(`wsymbol') xlabel(`xmin'(`xdis')`xmax', labsize(`xlabsize') grid) ylabel(`ymin'(`ydis')`ymax', axis(1) labsize(`ylabsize')))"
		local ii = `ii'+1
	...
	foreach aa in `agroups'{
		...
		local aplots = "`aplots' (line `yvar' `xvar' if age == `aa' & cohort25 >= ${yrfirst} & cohort25 <= `lyear', yaxis(1)  yscale(alt) yscale(alt axis(2)) lpattern(`wpatt') lcolor(`wcolor'))"
	...
	if "`otext'" == ""{
		...
		tw (bar rece year if `xvar' >= `xmin' & `xvar' <= `xmax', c(l) color(gray*0.5) yaxis(2) ytitle("",axis(2)) ylabel(,axis(2) nolabels noticks)  ) ///
		`aplots' `cplots' ///
		...
		legend(`off' size(`legsize') symxsize(7) ring(0) position(`posi') col(`colu') order(4 "`lab1'" 5 "`lab2'" 6 "`lab3'" ) ///
		region(color(none))) graphregion(color(white)) /// Legend options 
		graphregion(color(white)  ) ///				Graph region define
		plotregion(lcolor(black))  ///				Plot regione define
		title(`title', color(black) size(`titlesize')) subtitle(`subtitle', color(black) size(`subtitlesize'))  // Title and subtitle		
...
*	local otextl = "`52'"
	...
	*Plots
	...
	if "`otext'" == ""{
		...
		(pcarrowi `py1' `px1' `py2' `px2', msize(${legesize}) mlwidth(medthick) mlcolor(black) lcolor(black)) ///
		(pcarrowi `py1j' `px1j' `py2j' `px2j', msize(${legesize}) mlwidth(medthick) mlcolor(black) lcolor(black)) ///
		(pcarrowi `py1k' `px1k' `py2k' `px2k', msize(${legesize}) mlwidth(medthick) mlcolor(black) lcolor(black)), ///		
		text(`oy1' `ox1' "`otext'", place(w) size(${legesize})) ///
		text(`oy1j' `ox1j' "`otextj'", place(w) size(${legesize})) ///
		text(`oy1k' `ox1k' "`otextk'", place(w) size(${legesize})) ///
		text(`oy1l' `ox1l' "`otextl'", place(w) size(${legesize})) ///
		xtitle("`xlabby'", size(`xtitlesize')) ytitle("`ylabby'", size(`ytitlesize')) ///				
		legend(`off' size(`legsize') symxsize(7) ring(0) position(`posi') col(`colu') order(5 "`lab1'" 6 "`lab2'" 7 "`lab3'" 8 "`lab4'" ) ///
		region(color(none))) graphregion(color(white)) /// Legend options 
		graphregion(color(white)  ) ///				Graph region define
