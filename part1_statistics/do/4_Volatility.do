...
foreach yr of numlist $d1yrlist{
	...
	*
	...
	if inlist(`yrp',${perm3yrlist}) & inlist(`yr',${d5yrlist}){
			...
			_pctile permearn, p(97.5 99 99.5)
			
			replace permrank = 99 if permearn > r(r1) & permearn <= r(r2) & permearn !=. 
			replace permrank = 99.5 if permearn > r(r2) & permearn <= r(r3) & permearn !=. 
			...
			* Calculate "kurtosis" moments within permrank
			...
			bys agegp: egen aux3 = pctile(permearn), p(99.5)
			
			replace permrank = 99   if permearn > aux1 & permearn <= aux2 & permearn !=. 
			replace permrank = 99.5 if permearn > aux2 & permearn <= aux3 & permearn !=. 
			...
			bys male: egen aux3 = pctile(permearn), p(99.5)
			...
			* Calculate "kurtosis" moments within permrank/agegp 
			...
			bys male agegp: egen aux3 = pctile(permearn), p(99.5)
