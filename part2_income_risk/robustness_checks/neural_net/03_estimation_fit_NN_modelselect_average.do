** COMBINE NN MODEL SELECTION FILES (MEAN)
clear
set more off
...
** CONDITIONAL MEAN **
**********************
global resultdir "${maindir}/main_analysis/out/model_select_20210314_quantile_noclust_loglaginc"

* Make results folder
cap noisily mkdir "${maindir}/main_analysis/out/${c_date}/abs_pred_error/TS"
global outfolder "${maindir}/main_analysis/out/${c_date}/abs_pred_error/TS"
...
global tab6_col = `""Pois+noclust" "15" "16" "17" "18" "19""'
...
gen abs_pred_mean_TS = abs(tot_inc - cond_mean_TS)

* Gen likelihood of TS
gen llik_TS = tot_inc * log(cond_mean_TS) - cond_mean_TS
...
forvalues tabn = 1/3 {
	...
	* Loop over different specifications of NN
	...
	foreach specn in ${table`tabn'} {
		merge 1:1 person_id year using "${resultdir}/neuralnet_male_modelsel_poisson_spec`specn'_noclust.dta"
		drop _merge
		...
		* Loop over replications
		forvalues rep = 1/$nrep {
			replace cond_mean_ave = cond_mean_ave + (cond_mean_rep`rep' / $nrep)
		}
		
		* Compute abs_pred_mean and llik
		gen abs_pred_mean_ave = abs(tot_inc - cond_mean_ave)
		gen llik_ave = tot_inc * log(cond_mean_ave) - cond_mean_ave
		...
		* Prep for next iteration
		...
		drop cond_mean_rep* cond_mean_ave abs_pred_mean_ave llik_ave
	}
	
	* Save tables
	esttab matrix(abs_pred_mean_in, fmt(2)) ///
		using "${outfolder}/abs_pred_error_mean_in_tab`tabn'_average.tex", ///
		noobs nonumber nomtitles replace
	esttab matrix(abs_pred_mean_in, fmt(2)) ///
		using "${outfolder}/abs_pred_error_mean_in_tab`tabn'_average.csv", ///
		noobs nonumber nomtitles replace
		
	esttab matrix(abs_pred_mean_out, fmt(2)) ///
		using "${outfolder}/abs_pred_error_mean_out_tab`tabn'_average.tex", ///
		noobs nonumber nomtitles replace
	esttab matrix(abs_pred_mean_out, fmt(2)) ///
		using "${outfolder}/abs_pred_error_mean_out_tab`tabn'_average.csv", ///
		noobs nonumber nomtitles replace
	...
	* Save tables
	esttab matrix(abs_pred_mean_in_scaled, fmt(2)) ///
		using "${outfolder}/abs_pred_error_mean_in_scaled_tab`tabn'_average.tex", ///
		noobs nonumber nomtitles replace
	esttab matrix(abs_pred_mean_in_scaled, fmt(2)) ///
		using "${outfolder}/abs_pred_error_mean_in_scaled_tab`tabn'_average.csv", ///
		noobs nonumber nomtitles replace
		
	esttab matrix(abs_pred_mean_out_scaled, fmt(2)) ///
		using "${outfolder}/abs_pred_error_mean_out_scaled_tab`tabn'_average.tex", ///
		noobs nonumber nomtitles replace
	esttab matrix(abs_pred_mean_out_scaled, fmt(2)) ///
		using "${outfolder}/abs_pred_error_mean_out_scaled_tab`tabn'_average.csv", ///
		noobs nonumber nomtitles replace
}
