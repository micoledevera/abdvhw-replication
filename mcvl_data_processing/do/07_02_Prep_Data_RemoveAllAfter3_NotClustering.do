...
foreach i of numlist `start'/`end' {
	...
	**** Keep variables needed
	keep person_id firm_id firm_cc2 year month alta baja sex birth_year death_year education wage inkind ///
		age entryage MCVL_entry min_year max_year MCVL_last firm_muni contribution_regime merge_tax job_relationship ///
		contribution contract_type ptcoef person_muni_latest firm_jur_status
	...
	if `r(N)' > 0 {
		...
		**** Next, as a person_id-year panel
		...
		collapse (sum) wage inkind basque_navarra ///
				...
				birth_year death_year MCVL_entry min_year MCVL_last max_year public funcionario ///
				person_muni_latest permanent_main fulltime_main continuing_main govt_main firm_muni_main, ///
				by(person_id year)
		compress
...
save "$temp\observation_count.dta", replace
...
**** DEALING WITH ZEROS ****
use "$temp\mcvl_annual_panel_pt2", clear

gen remove = (added_zero_seq > 3 & !missing(added_zero_seq))
...
compress
sort person_id year
save "$out\mcvl_annual_FinalData_pt1_RemoveAllAfter3.dta", replace
...
gen est_sample = 0
replace est_sample = 1 if year != 2018

** Save
compress
sort person_id year
save "$out\mcvl_annual_FinalData_pt2_RemoveAllAfter3_NotClustering.dta", replace
